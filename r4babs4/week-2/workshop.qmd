...
🎬 Use the Files pane to make folders for the data. I suggest
...
-   **`flowAI`** [@flowAI]: which performs automated quality control by
    ...
    indicate a problem for the reading on a cell
...
you can access all software and filestores. When using the VDS for R and
RStudio, it usually makes sense to use other software - such as a
...
::: callout-tip
You don't have to use the names I have used for your own analysis if you
prefer different names. Simply edit the `meta.csv` file. However, you do
need to keep the Time column named that way for the next step to
proceed.
:::
...
The transformation is applied in two steps: the transformation needed is
estimated from the data and then that transformation is applied.
...
easier for you to use tools you already know and more easily follow the
priciples of what you are doing.
...
it by clicking on the dataframe in the Environment. At the moment, we
cannot tell which row (cell) is from which sample but we can add the
...
names have a specific format which we have matched with the regex. When
you work with your own data, the easist thing to do is ensure your files
have the same names as the same data.
:::
...
I chose theses values but you might judge the gate differently. You will
certainly need to use different values for your own data. The next plot
...
`clean_trans_live` now has `r nrow(clean_trans_live)` cells. We should
report how many cells were *in the gate* for each sample.
...
and the antibody and calculate what % cells remained in each sample
after gating (*i.e.*, the % live cells):
...
From now on we are working with the cleaned and gated data, that is, the
dataframe `clean_trans_live`.

We have two signals:

-   `TNFa_APC_Lin` should indicate the amount of TNF-α protein in the
    cell
...
🎬 We can add the number of live cells to this data frame and calculate
the percentage of live cells that are positive for TNF-α:
...
You may wish to repeat the process we used for the `TNFa_APC_Lin` signal
to find out the number of cells that are FITC positive in each sample
and the mean intensity of the FITC signal in the FITC positive cells.
:::

### A figure to report baselines for both signals

🎬 Plot `TNFa_APC_Lin` against `E_coli_FITC_Lin` with the thresholds for
both signals:
