...
# import % live summary
clean_trans_live_tfna_pos <- read_csv("data-processed/clean_trans_live_tfna_pos.csv") 
...
clean_trans_live_tfna_pos |> 
  knitr::kable(caption = "clean_trans_live_tfna_pos: sample summaries") |> 
  kableExtra::kable_styling() 
...
clean_trans_live |> 
  ...
  geom_vline(xintercept = apc_cut, 
             color = "red") +
  geom_text(data = clean_trans_live_tfna_pos |> 
              filter(treatment == "MEDIA",
                     antibody == "ISOTYPE"), 
...
all_combin_perc <- all_combin_n |> 
  left_join(clean_trans_live_tfna_pos, 
...
The [BIO00066I Biomedical Sciences class data](https://docs.google.com/spreadsheets/d/1aMWvzRRT86SWFuCGaO_yjnbeH3XtxralaKsO5ffVMfE/edit?usp=drive_link) are in a google sheet.
...
```{r}
#| eval: false
file <- "https://docs.google.com/spreadsheets/d/1aMWvzRRT86SWFuCGaO_yjnbeH3XtxralaKsO5ffVMfE/edit?usp=drive_link"
```

```{r}
#| eval: false
class_data <- read_sheet(file, sheet = "data-2026")
```
