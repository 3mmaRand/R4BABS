...
There are several possible post-hoc tests and we will be using Tukey's HSD (honestly significant difference) test [@tukey1949]implemented in the **`emmeans`** package.
...
Each row is a comparison between means.
The 'estimate' column is the difference between the means and the 'p.value' column indicates whether that difference is significant.
...
To check these we need to calculate the residuals and we can do examine the `mod$residuals` variable.
...
Alternatively, [View in Browser](https://github.com/3mmaRand/R4BABS/blob/main/r4babs2/week-4/workshop.qmd).Coding and thinking answers are marked with `#---CODING ANSWER---` and `#---THINKING ANSWER---`
