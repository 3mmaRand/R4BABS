...
dunn_tidy <- dunnTest(data = sweat, na ~ gp)$res

p_FA_v_UU <- dunn_tidy |> 
  filter(Comparison == "FA - UU") |> 
  pull(P.adj) |> round(4)
p_FU_v_UU <- dunn_tidy |> 
  filter(Comparison == "FU - UU") |> 
  pull(P.adj) |> round(4)
...
ggplot(sweat, aes(x = gp, y = na) ) +
  ...
  annotate("text", x = 2,  y = 103, 
           label = glue::glue("italic(p) == { p_FA_v_UU }"),
           parse = TRUE) +
  # FU - UU
  ...
  annotate("text", x = 2.5,  y = 93, 
           label = glue::glue("italic(p) == { p_FU_v_UU }"),
           parse = TRUE) +
  theme_classic()
...
# Normally we don't mark NS results but here I have because it is an 
# important result - E is useless
...
ggplot() +
  ...
  annotate("text", x = 5.5,  y = 388, 
           label = "N.S", size = 4) +
 
   # D - WaterControl    ***
  annotate("segment", x = 4, xend = 6, 
           y = 463, yend = 463,
           colour = "black") +
  annotate("text", x = 4.5,  y = 466, 
           label = "***", size = 5) +
  ...
  annotate("segment", x = 2, xend = 6, 
           y = 499, yend = 499,
           colour = "black") +
  annotate("text", x = 3.5,  y = 502, 
           label = "***", size = 5) +

    # WaterControl-A ***
  annotate("segment", x = 1, xend = 6, 
           y = 517, yend = 517,
           colour = "black") +
  ...
  annotate("segment", x = 2, xend = 5, 
           y = 426, yend = 426,
           colour = "black") +
  annotate("text", x = 3.5,  y = 429, 
           label = "***", size = 5) +    
  
  #  A - E ***
  annotate("segment", x = 1, xend = 5, 
           y = 442, yend = 442,
           colour = "black") +
  annotate("text", x = 3,  y = 445, 
           label = "***", size = 5) + 
