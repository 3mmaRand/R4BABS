...
`confidence-intervals.R` to carry out the rest of the work.

![](images/do_in_R.png) Add a comment to the script:
`# confidence intervals: large and small samples` and load the
...
In the BABS 1 week 8 workshop we [summarised the mass of
cats](../../r4babs1/week-8/workshop.html#cat-mass) using this method.
You can look back at that if you need to.
:::
...
The value returned by `qt()` is larger than that by `qnorm()` which
