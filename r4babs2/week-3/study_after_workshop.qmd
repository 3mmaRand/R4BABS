...
# If you want to extract the p value using method 2 you can
# do it like this

p_value <- broom::glance(mod) |> pull(p.value) |> round(4)
# However, because the p value is so small this will round to zero.
# We don't want to display p = 0 but something like p < 0.001 
# instead. We can do this with an ifelse statement. If the 
# p value is less than 0.001 we will display "< 0.001" otherwise we
# will round the p value to 3 decimal places.
p_value <- broom::glance(mod) |> pull(p.value) |> 
