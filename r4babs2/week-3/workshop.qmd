...
![Allison Horst cartoon: How much I think I know about R"](images/how-much-i-know.png){fig-alt="Illustrated line plot of 'How much I think I know about R' on the y-axis, and 'Time' on the x-axis. Along the line are emoji-style faces, showing the non-linear progression of R knowledge over time. At first, a nervous face becomes a happy face early on in learning, then a "grimace" face at an intermediate peak before a steep decline (with an exhausted face at the local minimum). Then, a determined face charges back up a hill, reaching another peak with a 'mind blown' face and text annotation 'join R community on twitter' followed by another decline, but this time the faces look happy even though their 'How much I think I know about R' value is declining." width="800"}
...
![](images/do_in_R.png) Make an RStudio project for this workshop by clicking on the drop-down menu on top right where it says `Project: (None)` and choosing New Project, then New Directory, then New Project. Navigate to the `data-analysis-in-r-2` folder and name the RStudio Project `week-3`.
...
The assumptions of the general linear model are that the residuals – the difference between predicted value (i.e., the group mean) and observed values - are normally distributed and have homogeneous variance. To check these we need to calculate the residuals and we can do examine the `mod$residuals` variable. You may want to refer to [Checking assumptions](../week-2/workshop.html#checking-assumptions) in Week 2's workshop.
...
![](images/do_in_R.png) Create a figure like the one below. You may need to the [Visualise](../../r4babs1/week-9/workshop.html#visualise) from the Week 9 workshop of BABS1 [@rand2023]
...
We now need to annotate the figure with the results from the test.
...
![](images/do_in_R.png) Format the figure so it is more suitable for a report.  
...
![](images/do_on_your_computer.png) Save a copy of [coliexp.txt](data-raw/coliexp.txt)

![](images/do_in_R.png) Read the data in
```{r geneimport, echo = FALSE}
#| include: false  #---CODING ANSWER---
# I have my data files in a folder called data
coliexp  <-  read_table("data-raw/coliexp.txt")
```
...
![](images/do_in_R.png) Now carry out a paired-sample *t*-test. 
```{r genepairedt}
...
![](images/future_you.png){fig-alt="xxx" width="500"}
