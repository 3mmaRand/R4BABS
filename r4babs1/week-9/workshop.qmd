...
`figures/adipocytes.png` is the name of the file including the relative path. ![](images/do_in_R.png) Look up `ggsave()` in the manual to understand the arguments. You can do this by putting your cursor on the command and pressing F1
...
![Interorbital width is the distance between the eyes](images/interorbital.png){fig-alt="Photo of a domestic pigeon, head on, with an arrow showing the distance between the eyes."} \### Import
...
Pages made with Quarto @allaire2022 using the following packages: knitr @knitr, tidyverse @tidyverse-2, kableExtra @kableExtra.
