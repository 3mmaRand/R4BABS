...
![](images/do_in_R.png) Make an RStudio project for this workshop by clicking on the drop-down menu on top right where it says `Project: (None)` and choosing New Project, then New Directory, then New Project. Name the RStudio Project 'week-8'.
...
[^2]: **Don't** be tempted to import data this way. Unless you are careful, your data import will not be scripted or will not be scripted correctly.
...
This sort of data might be represented with a barchart. You have two options for producing that barchart: 1. plot the summary table using `geom_col()`

2.  plot the raw data using `geom_bar()`

colour of cats dplyr table bar chart
...
As the interquartile is `IQR(fly_bristles$number)` and the median is `median(fly_bristles$number)` then 25% flies have `median(fly_bristles$number) - IQR(fly_bristles$number)/2` bristles or fewer and 25% have `median(fly_bristles$number) + IQR(fly_bristles$number)/2` or more.
...
cat mass - a a single variable histogram - effect of changing bins dplyr table mean, median,
...
![](images/do_in_R.png) The `group_by()` function is used before the summarise() to do calculations for each of the coats:
```{r}
cats |> 
  group_by(coat) |> 
  summarise(mean = mean(mass),
...
You can read this as:
  ...
  > take cats *and then*
  > group by coat *and then*
  > summarise by finding the mean of mass and the standard deviation of mass
...
![Not normal](images/not_normal.png){fig-alt="two distributions as characters. One is normal (bell-shaped) the other is not"}
