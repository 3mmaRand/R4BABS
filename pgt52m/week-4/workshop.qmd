...
## Data already tidy: Myoglobin in seals or [Psych Example](psych-example.html)
...
-   this example, one continuous response and one nominal explanatory
    variable with three groups ([seals](data-raw/seal.csv))
...
## Data needs tidying: Pigeons {#pigeons-w4}
...
eyes](images/interorbital.png){fig-alt="Photo of a domestic pigeon, head on,   with an arrow showing the distance between the eyes."}
...
Tidy format has variables in column and observations in rows. All of the
distance measurements should be in one column and a second column should
give the population.
...
Future you is going to summarise and plot data from other modules. You
can make this much easier by documenting what you have done now. Go
through your script (`summarise-plot-data-with-several-vars.R`) and
remove code that you do not need. Add comments to the code you do need
