...
section](../week-4/workshop.html#summarise) from the Week 4
workshop [@rand2023]
...
to refer to [Visualise](../week-4/workshop.html#visualise)
...
You can annotate the *p*-value manually by typing it in to the appropriate
`annotate()` function. 
...
-   `annotate("segment", ...)` adds a line to the figure starting at
    *x = 1* and *y = 11.3* and ending at *x = 2* and *y = 11.3*
-   `annotate("text", ...)` adds text to the figure at *x = 1.5* and
    *y = 11.7*
...
-   `parse = TRUE` tells ggplot2 to interpret that string as a
    plotmath expression rather than literally adding "italic(p) == 0.0028". 
...
-   **`broom** is a package that turns model outputs into more easy to use tidy 
    formats
-   **`glance(mod)`** puts information from mod including the *p*-value into a 
    dataframe
-   `pull(p.value)` extracts the value from *p*-value column in the dataframe
...
-  The glue package allows us to use R objects in strings so their v
   values are printed.
-  We use curly braces `{}` a round the R object so it is understood as an 
   R object and its value is written rather than literally writing "p_value"
...
This presents you with less information. The *p*-value for the 
effect of temperate on gene expression will always be the same as in 
the larger table.
